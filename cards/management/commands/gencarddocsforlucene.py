...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['clustertest']:
            d_pcard_list = PhysicalCard.objects.filter(Q(id__lte=15876), Q(basecard__rules_text__icontains='destroy'), Q(basecard__rules_text__icontains='target'), Q(basecard__rules_text__icontains='creature')).order_by('-basecard__card__multiverseid')[:25]
            land_pcard_list = PhysicalCard.objects.filter(Q(id__lte=15876), Q(basecard__cardtype__type__type='Land')).order_by('-basecard__card__multiverseid')[:50]
            gl_pcard_list = PhysicalCard.objects.filter(Q(id__lte=15876), Q(basecard__rules_text__icontains='gain'), Q(basecard__rules_text__icontains='life')).order_by('-basecard__card__multiverseid')[:25]
            flying_pcard_list = PhysicalCard.objects.filter(Q(id__lte=15876), Q(basecard__rules_text__contains='Flying'), Q(basecard__cardtype__type__type='Creature')).order_by('-basecard__card__multiverseid')[:50]
            search_pcard_list = PhysicalCard.objects.filter(Q(id__lte=15876), Q(basecard__rules_text__icontains='search'), Q(basecard__cardtype__type__type='Creature')).order_by('-basecard__card__multiverseid')[:50]
            counter_pcard_list = PhysicalCard.objects.filter(Q(id__lte=15876), Q(basecard__rules_text__icontains='counter'), Q(basecard__rules_text__icontains='target'), Q(basecard__rules_text__icontains='spell')).order_by('-basecard__card__multiverseid')[:25]
            burn_pcard_list = PhysicalCard.objects.filter(Q(id__lte=15876), Q(basecard__rules_text__icontains='deal'), Q(basecard__rules_text__icontains='damage'), Q(basecard__rules_text__icontains='target'), Q(basecard__rules_text__icontains='creature')).order_by('-basecard__card__multiverseid')[:25]
            artifact_pcard_list = PhysicalCard.objects.filter(Q(id__lte=15876), Q(basecard__rules_text__icontains='{t}'), Q(basecard__cardtype__type__type='Artifact')).order_by('-basecard__card__multiverseid')[:25]
            pcard_list = itertools.chain(d_pcard_list, land_pcard_list, gl_pcard_list, flying_pcard_list, search_pcard_list, counter_pcard_list, burn_pcard_list, artifact_pcard_list)
        ...
        for pcard in pcard_list:
            ...
            if options['clustertest']:
                ...
                clustertest_csv_out.write('"{}","{}",{},{},{},{},{},{},{},{},"{}",{},{},"http://card.ninja/cards/{}/"\n'.format(pcard.get_card_name(), bcard.mana_cost, bcard.cmc, white in bccolors, blue in bccolors, black in bccolors, red in bccolors, green in bccolors, ' '.join([t.type for t in bcard.types.all()]), ' '.join([t.subtype for t in bcard.subtypes.all()]), rt, bcard.power, bcard.toughness, pcard.get_latest_url_part()))
            else:
