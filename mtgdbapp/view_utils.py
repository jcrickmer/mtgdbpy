...
def convertSymbolsToHTML(text):
	...
	# Zoinks. probably need to implement something like this to match upper-case but convert to lower on output: http://stackoverflow.com/questions/2643737/how-to-replace-by-regular-expression-to-lowercase-in-python

	def format_mana_terms(term0, term1):
		return tag_open + 'class="magic-symbol-small symbol_mana_' + term0.lower() + term1.lower() + '_small" alt="{' + term0.lower() + term1.lower() + '}"' + tag_close
	def format_mana_term(term0):
		return tag_open + 'class="magic-symbol-small symbol_mana_' + term0.lower() + '_small" alt="{' + term0.lower() + '}"' + tag_close
	#def format_term(term0):
	#	return tag_open + 'class="magic-symbol-small symbol_' + term0.lower() + '_small" alt="{' + term0.lower() + '}"' + tag_close

	pattern2 = re.compile(r'\{([WUBRG2])\/?([WUBRGP])\}', re.IGNORECASE)
	result = pattern2.sub(lambda m: format_mana_terms(m.group(1), m.group(2)), result)
